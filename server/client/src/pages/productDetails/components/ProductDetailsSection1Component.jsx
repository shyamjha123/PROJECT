...
import {
  ...
  cartListProduct,
  updateQuantity,
} from "src/shared/apiCall/cart";
import { getSingleProduct } from "src/shared/apiCall/product";
import { isWish, isWishReq, unWishReq } from "src/shared/apiCall/wish";
import { totalCost } from "src/shared/helpers/totalCost";
import { slideImage } from "../helpers/slideImage";
import ProductDetailsSection2Component from "./ProductDetailsSection2Component";
import Submenu from "./Submenu";

const ProductDetailsSection1Component = () => {
  let { id } = useParams();
    ...
    try {
      ...
      setTimeout(() => {
        if (responseData.Product) {
          dispatch(singleProductDetail(responseData.Product)); // Dispatch in layout context
          setPimages(responseData.Product.pImages);
          dispatch(loading(false));
          dispatch(inCart(cartList())); // This function change cart in cart state
        }
  ...
  useEffect(() => {
    if (homeSlice.inCart && homeSlice.inCart !== null) {
      let cart = localStorage.getItem("cart")
        ...
        : [];
      // loop through every cart and pick the inCart object
      if (cart.length > 0) {
        cart.forEach(item => {
          // id :   product id
  ...
  return (
    <>
            ...
            <div className="my-4 md:my-6">
              ...
              >
                ...
                {sProduct.pQuantity !== 0 ? (
                  <>
                    {homeSlice.inCart == null ||
                    (homeSlice.inCart !== null &&
                      homeSlice.inCart.includes(sProduct._id) === false) ? (
              ...
              {sProduct.pQuantity !== 0 ? (
                <>
                  {homeSlice.inCart !== null &&
                  homeSlice.inCart.includes(sProduct._id) === true ? (
                    <div
      ...
      <ProductDetailsSection2Component />
    </>
  );
};

export default ProductDetailsSection1Component;
